        ...
        for step in range(self.episode_length):
            ...
            for current_agent_id in range(len(self.policy_agents[p_id])):
                ...
                if step == 0:
                    ...
                    turn_dones_last[:, current_agent_id] = turn_dones[:, current_agent_id]
                    turn_dones_env_last = turn_dones_env.copy()
                    turn_next_obs_last[:,current_agent_id] = obs[:, current_agent_id]
                    turn_next_share_obs_last[:,current_agent_id] = share_obs[:, current_agent_id]
                ...
                # done=True env
                reset_choose[dones_env == True] = True
                ...
                turn_dones[dones_env == True, left_agent_id:] = np.ones(((dones_env == True).sum(), left_agents_num, 1)).astype(np.float32)
                ...
                turn_rewards_since_last_action[dones_env ==True, left_agent_id:] = 0.0

                # use any value is okay
                turn_acts[dones_env == True, left_agent_id:] = np.zeros(((dones_env == True).sum(), left_agents_num, policy.act_dim))
                turn_obs[dones_env == True, left_agent_id:] = np.zeros(((dones_env == True).sum(), left_agents_num, policy.obs_dim))
                turn_share_obs[dones_env == True, left_agent_id:] = np.zeros(((dones_env == True).sum(), left_agents_num, policy.central_obs_dim))
                turn_avail_acts[dones_env == True, left_agent_id:] = np.ones(((dones_env == True).sum(), left_agents_num, policy.act_dim))

                # done==False env
                # deal with current agent
                turn_dones[dones_env == False, current_agent_id] = dones_original[dones_env == False, current_agent_id].copy()
                turn_dones_env[dones_env ==False] = dones_env_original[dones_env == False].copy()
            ...
            new_next_obs, new_next_share_obs, new_next_avail_acts = env.reset(reset_choose)
