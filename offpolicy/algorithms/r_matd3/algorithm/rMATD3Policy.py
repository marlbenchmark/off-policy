...
class R_MATD3Policy:
    def __init__(self, config, policy_config, train=True):
        ...
        self.actor = R_Actor(self.args, self.obs_dim, self.act_dim,
                             self.discrete_action, self.device, take_prev_action=self.prev_act_inp)
        self.critic = R_Critic(
            self.args, self.central_obs_dim, self.central_act_dim, self.device)

        self.target_actor = R_Actor(self.args, self.obs_dim, self.act_dim,
                                    self.discrete_action, self.device, take_prev_action=self.prev_act_inp)
    ...
    def get_actions(self, obs, prev_actions, actor_rnn_states, available_actions=None, t_env=None, explore=False,
        ...
        if use_target:
            ...
            actor_out, new_rnn_states = self.actor(
                obs, prev_actions, actor_rnn_states)
