...
class R_MADDPG:
    ...
    def shared_train_policy_on_batch(self, update_policy_id, batch, update_actor=None):
        ...
        error = predicted_Q_sequence - target_Q_sequence.float().detach()
        if self.use_per:
            # prioritized experience replay
            if self.use_huber_loss:
                per_batch_critic_loss = huber_loss(
            ...
            importance_weight_critic_loss = per_batch_critic_loss * \
            ...
            td_errors = error.abs().detach().numpy()
            ...
            if self.use_huber_loss:
                critic_loss = huber_loss(
                    error, self.huber_delta).sum() / (1 - curr_env_dones).sum()
            else:
                critic_loss = mse_loss(error).sum() / \
    ...
    def cent_train_policy_on_batch(self, update_policy_id, batch, update_actor=None):
        ...
        if self.use_value_active_masks:
            ...
            predicted_Q_sequence = predicted_Q_sequence * \
                (1 - curr_agent_dones)
        ...
        if self.use_per:
            ...
            if self.use_huber_loss:
                ...
                per_batch_critic_loss = mse_loss(error).sum(dim=0).flatten()
            agent_importance_weight_critic_loss = per_batch_critic_loss * \
                torch.FloatTensor(agent_importance_weights)

            if self.use_value_active_masks:
                critic_loss = agent_importance_weight_critic_loss.sum(
                ) / ((1 - curr_env_dones) * (1 - curr_agent_dones)).sum()
            else:
                critic_loss = agent_importance_weight_critic_loss.sum() / (1 - curr_env_dones).sum()

            td_errors = error.abs().detach().numpy()
            agent_new_priorities = ((1 - self.args.per_nu) * td_errors.mean(
                axis=0) + self.args.per_nu * td_errors.max(axis=0)).flatten()
            ...
            if self.use_huber_loss:
                ...
                if self.use_value_active_masks:
                    ...
                    critic_loss = mse_loss(
                        error).sum() / (1 - curr_env_dones).sum()
            new_priorities = None
