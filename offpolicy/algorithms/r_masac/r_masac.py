...
class R_MASAC:
    ...
    def get_update_info(self, update_policy_id, obs_batch, act_batch, nobs_batch, avail_act_batch, navail_act_batch):
        ...
        for p_id in self.policy_ids:
            ...
            with torch.no_grad():
                ...
                pol_nact_seq, _, pol_nact_log_probs = policy.get_actions(combined_nobs_seq_batch,
                ...
                ind_agent_nact_log_probs = pol_nact_log_probs.split(
                    split_size=batch_size, dim=1)
    ...
    def shared_train_policy_on_batch(self, update_policy_id, batch, update_actor=None):
        ...
        with torch.no_grad():
            for t in range(self.episode_length):
                ...
                if self.use_popart:
                    next_Q_t = self.value_normalizer[update_policy_id].denormalize(
                        next_Q_t)
        ...
        next_step_V_sequence = (
            1 - env_done_sequence.float()) * next_step_V_sequence
        target_Q_sequence = (
            rew_sequence + self.args.gamma * next_step_V_sequence).float()
        if self.use_popart:
        ...
        for t in range(self.episode_length):
            ...
            Q_t_1, Q_t_2, _ = update_policy.critic(
                stacked_cent_obs_seq[t], replaced_cent_act_batch, critic_rnn_state)
            _, _, critic_rnn_state = update_policy.critic(
        ...
        if self.args.automatic_entropy_tune:
            ...
            if alpha_loss_sequence.shape[-1] > 1:
                alpha_loss_sequence = alpha_loss_sequence.mean(
                    dim=-1).unsqueeze(-1)
            ...
            ent_diff = (policy_log_prob_seq +
                        update_policy.target_entropy).detach().mean()
        else:
    ...
    def cent_train_policy_on_batch(self, update_policy_id, batch, update_actor=None):
        ...
        next_steps_dones = all_env_dones[:
                                         self.episode_length - 1, :, :].float()
        curr_env_dones = torch.cat((first_step_dones, next_steps_dones), dim=0)

        predicted_Q1_sequence, predicted_Q2_sequence, _ = update_policy.critic(
        ...
        with torch.no_grad():
            ...
            for t in range(self.episode_length):
                #  update the RNN states based on the buffer sequence
                _, _, target_critic_rnn_state = update_policy.target_critic(
                    all_agent_cent_obs[t], all_agent_cent_act_buffer[t], target_critic_rnn_state)
                next_Q1_t, next_Q2_t, _ = update_policy.target_critic(
        ...
        # mask the next step Vs and form bootstrapped targets
        next_step_V_sequence = (
            1 - all_env_dones.float()) * next_step_V_sequence
        
        target_Q_sequence = (all_agent_rewards +
                             self.args.gamma * next_step_V_sequence).float()
        if self.use_popart:
            nodones_target_Q_sequence = target_Q_sequence[curr_env_dones == 0]
            target_Q_sequence[curr_env_dones == 0] = self.value_normalizer[update_policy_id](
                nodones_target_Q_sequence)
