...
from offpolicy.algorithms.r_masac.algorithm.r_actor_critic import R_DiscreteActor, R_GaussianActor, R_Critic
...
class R_MASACPolicy:
    def __init__(self, config, policy_config, train=True):
        ...
        if self.discrete_action:
            self.actor = R_DiscreteActor(
                self.args, self.obs_dim, self.act_dim, self.device, take_prev_action=self.prev_act_inp)
            # slightly less than max possible entropy
            self.target_entropy = - \
                np.log((1.0 / self.act_dim)) * self.target_entropy_coef

        else:
            self.actor = R_GaussianActor(self.args, self.obs_dim, self.act_dim,
                                         self.act_space, self.device, take_prev_action=self.prev_act_inp)
            # max possible entropy
            self.target_entropy = - \
                torch.prod(torch.Tensor(self.act_space.shape)).item()

        self.critic = R_Critic(self.args, self.central_obs_dim,
                               self.central_act_dim, self.device, discrete=False)
        self.target_critic = R_Critic(
            self.args, self.central_obs_dim, self.central_act_dim, self.device, discrete=False)
    ...
    def get_actions(self, obs, prev_actions, actor_rnn_states, available_actions=None, sample=True, sample_gumbel=False):
        ...
        assert prev_actions is None or len(
            obs.shape) == len(prev_actions.shape)

        if sample:
            if self.discrete_action:
                # actions are the sample output
                actions, log_probs, _, rnn_outs = self.actor.sample(
                    obs, prev_actions, actor_rnn_states, available_actions, sample_gumbel=sample_gumbel)

            else:
                # actions are the sample output
                actions, log_probs, _, rnn_outs = self.actor.sample(
                    obs, prev_actions, actor_rnn_states, available_actions)
        else:
            # actions are the mode of the distribution
            _, log_probs, actions, rnn_outs = self.actor.sample(
                obs, prev_actions, actor_rnn_states, available_actions)

        return actions, rnn_outs, log_probs
