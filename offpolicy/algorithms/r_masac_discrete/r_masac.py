...
class R_MASAC:
    ...
    def get_update_info(self, update_policy_id, obs_batch, act_batch, nobs_batch, avail_act_batch, navail_act_batch):
        ...
        for p_id in self.policy_ids:
            ...
            with torch.no_grad():
                ...
                pol_nact_seq, (pol_nact_probs, pol_nact_log_probs), _ = policy.get_actions(combined_nobs_seq_batch,
                ...
                ind_agent_nact_probs = pol_nact_probs.cpu().split(split_size=batch_size, dim=1)
                ind_agent_nact_log_probs = pol_nact_log_probs.cpu().split(split_size=batch_size, dim=1)

            if p_id == update_policy_id:
                update_policy_nact_probs = ind_agent_nact_probs
        ...
        all_agent_nact_probs = torch.stack(update_policy_nact_probs).sum(dim=0) / len(self.policy_agents[update_policy_id])

        return cent_act_sequence_critic, act_sequences, act_sequence_replace_ind_start, cent_nact_sequence, all_agent_nact_log_probs, update_policy_nact_log_probs, all_agent_nact_probs, update_policy_nact_probs
    ...
    def shared_train_policy_on_batch(self, update_policy_id, batch, update_actor=None):
        ...
        cent_act_sequence_buffer, act_sequences, act_sequence_replace_ind_start, cent_nact_sequence, all_agent_nact_log_prob_seq, _, all_agent_nact_prob_seq, _ = \
        ...
        with torch.no_grad():
            for t in range(self.episode_length):
                ...
                nact_probs = all_agent_nact_prob_seq[t]
                if nact_log_probs.shape[-1] > 0:
                    nact_log_probs = nact_log_probs.mean(dim=-1).unsqueeze(-1)
                if nact_probs.shape[-1] > 0:
                    nact_probs = nact_probs.mean(dim=-1).unsqueeze(-1)
                ...
                next_step_V = nact_probs * next_step_V
        ...
        policy_act_seq, (policy_prob_seq, policy_log_prob_seq), _ = update_policy.get_actions(pol_agents_obs_seq,
        ...
        if self.args.automatic_entropy_tune:
            ...
            alpha_loss_sequence = -(update_policy.log_alpha * (policy_log_prob_seq * policy_prob_seq + update_policy.target_entropy).detach())
    ...
    def cent_train_policy_on_batch(self, update_policy_id, batch, update_actor=None):
        ...
        # get centralized sequence information: cent_obs_sequence is tensor of shape (ep_len, batch_size, cent obs dim)
        cent_act_sequence_buffer, act_sequences, act_sequence_replace_ind_start, cent_nact_sequence, all_agent_nact_log_prob_seq, update_pol_nact_logprobs, all_agent_nact_prob_seq, update_pol_nact_probs = \
            self.get_update_info(update_policy_id, obs_batch, act_batch,
                                 nobs_batch, avail_act_batch, navail_act_batch)
        ...
        all_nact_probs = torch.cat(update_pol_nact_probs, dim=1)
        ...
        with torch.no_grad():
            ...
            for t in range(self.episode_length):
                ...
                nact_probs = all_nact_probs[t]
                if nact_log_probs.shape[-1] > 1:
                    nact_log_probs = nact_log_probs.mean(dim=-1).unsqueeze(-1)
                if nact_probs.shape[-1] > 1:
                    nact_probs = nact_probs.mean(dim=-1).unsqueeze(-1)
                if self.use_popart:
                    next_Q_t = self.value_normalizer[update_policy_id].denormalize(next_Q_t)
                next_step_V = next_Q_t - (update_policy.alpha * nact_log_probs)
