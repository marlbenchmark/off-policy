...
import numpy as np
from offpolicy.algorithms.qmix.algorithm.vdn_mixer import VDNMixer
...
class VDN:
    ...
    def train_policy_on_batch(self, batch, use_same_share_obs, update_popart):
        ...
        for p_id in self.policy_ids:
            ...
            pol_prev_act_buffer_seq = torch.cat((torch.zeros(1, total_batch_size, sum_act_dim),
                                                 stacked_act_batch[:-1]))
            pol_all_q_out_sequence, pol_final_hidden = policy.get_q_values(
            ...
            if isinstance(pol_all_q_out_sequence, list):
                ...
                for i in range(len(policy.act_dim)):
                    ...
                    curr_pol_q_out_sequence = torch.gather(
                        curr_all_q_out_sequence, 2, curr_stacked_act_batch_ind.unsqueeze(dim=-1))
                ...
                pol_q_out_sequence = torch.gather(
                    pol_all_q_out_sequence, 2, stacked_act_batch_ind.unsqueeze(dim=-1))
            ...
            with torch.no_grad():
                ...
                _, new_target_hiddens = target_policy.get_q_values(stacked_obs_batch[0], torch.zeros(
                    total_batch_size, sum_act_dim), target_policy.init_hidden(-1, total_batch_size))

                if self.args.use_double_q:
                    # actions come from live q; get the q values for the final nobs
                    pol_final_qs, _ = policy.get_q_values(
                        stacked_nobs_batch[-1], stacked_act_batch[-1], pol_final_hidden)
                    ...
                    if type(pol_final_qs) == list:
                        ...
                        for i in range(len(pol_final_qs)):
                            ...
                            pol_curr_nacts = make_onehot(
                                pol_curr_nacts, curr_act_dim, seq_len=seq_len)
                        ...
                        pol_all_nq_out_sequence_out = torch.cat(
                        ...
                        pol_nacts = pol_all_nq_out_sequence_detach.max(
                            dim=-1)[1]
                        if isinstance(policy.act_dim, np.ndarray):
                            # multidiscrete case
                            sum_act_dim = int(sum(policy.act_dim))
                        ...
                        pol_nacts = make_onehot(
                            pol_nacts, sum_act_dim, seq_len=seq_len)
