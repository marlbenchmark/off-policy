...
class MultiEnv(MultiAgentEnv, Env):
    ...
    def reset(self, new_inflow_rate=None):
        ...
        if self.env_params.get_additional_param('warmup_fixed_agents'):
            num_agents = self.env_params.get_additional_param('num_agents')
            curr_num_agents = len(self.k.vehicle.get_rl_ids())
            t = 0
            while curr_num_agents != (num_agents):
